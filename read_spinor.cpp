...
Eigen::MatrixXcd readMos(std::vector<double> &epsilon) {
	...
	std::ifstream spinorRe("mos");
	if (spinorRe.fail()) {
		std::cout << "\nWARNING: could not read file mos ...\n"
	...
	iss >> fmt;
	if (fmt != "$scfmo") {
		std::cout << "\nWARNING: expected flag '$scfmo' but got: " << fmt << "\n"
	...
	while (getline(spinorRe, line)) {
		...
		for (int mu=0; mu<=nAO/coeffPerLine; mu++) {
			getline(spinorRe, line);
			...
			for (int k=0; k<coeffPerLine && 2*nAO>coeffPerLine*mu+k; k++) {
				...
				s(i, coeffPerLine*mu + k) = stod(line.substr(coeffLength*k, coeffLength)), 0;
			}
			if (nAO == coeffPerLine * (mu+1)) break;
		}
		i++;
	}
	spinorRe.close();
